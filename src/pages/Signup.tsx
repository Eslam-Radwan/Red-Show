...
const Signup = () => {
    ...
    return (
        <div className="flex bg-black">
            ...
            <div className={`transition duration-500 ${isLogin? 'flex-2/3':'hidden'}`}>
            ...
            <div className={`${isLogin? "hidden":'flex-2/3 flex gap-4 flex-col h-dvh rounded-l-4xl bg-white justify-center items-center'} `}>
                ...
                <Form onSubmit={handleSubmit} method="post" className="grid gap-5 justify-center items-center content-center">
                    ...
                    <p className="w-fit col-span-2 justify-self-center">already have an accound <Link className="underline" to='/login' onClick={() => setIsLogin(true)}>Login</Link></p>
                </Form>
            </div>
            <span className={`transition-opacity duration-1000 ${isError === true ? "" : "hidden"}`}>
                <Modal message={actionData?.error} color="red" />
            </span>
        </div>
